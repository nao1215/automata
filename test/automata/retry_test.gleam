...
fn ms(milliseconds: Int) -> retry.Duration {
  let assert Ok(d) = retry.from_milliseconds(milliseconds: milliseconds)
  d
}

pub fn from_milliseconds_rejects_negative_test() {
  retry.from_milliseconds(milliseconds: -1)
  |> should.equal(
    Error(retry.NegativeDuration(unit: "milliseconds", actual: -1)),
  )
}
...
pub fn from_milliseconds_rejects_overflow_test() {
  ...
  retry.from_milliseconds(milliseconds: just_over_safe)
  |> should.equal(
    Error(retry.DurationOverflow(unit: "milliseconds", actual: just_over_safe)),
  )
}

pub fn from_seconds_converts_to_milliseconds_test() {
  let assert Ok(d) = retry.from_seconds(seconds: 30)
  retry.duration_milliseconds(duration: d) |> should.equal(30_000)
}

pub fn from_minutes_converts_to_milliseconds_test() {
  let assert Ok(d) = retry.from_minutes(minutes: 2)
  retry.duration_milliseconds(duration: d) |> should.equal(120_000)
}

pub fn duration_to_string_renders_milliseconds_test() {
  retry.duration_to_string(duration: ms(750))
  |> should.equal("750ms")
}

pub fn fixed_rejects_zero_max_attempts_test() {
  retry.fixed(delay: ms(100), max_attempts: 0)
  |> should.equal(Error(retry.MaxAttemptsMustBePositive(actual: 0)))
}

pub fn exponential_rejects_zero_initial_test() {
  retry.exponential(initial: ms(0), multiplier: 2, max_attempts: 5)
  |> should.equal(
    Error(retry.InitialDelayMustBePositive(actual_milliseconds: 0)),
  )
}

pub fn exponential_rejects_multiplier_below_two_test() {
  let assert Ok(d) = retry.from_milliseconds(milliseconds: 100)
  retry.exponential(initial: d, multiplier: 1, max_attempts: 5)
  |> should.equal(Error(retry.MultiplierMustBeAtLeastTwo(actual: 1)))
}
...
pub fn capped_exponential_rejects_cap_below_initial_test() {
  ...
  |> should.equal(
    Error(retry.CapMustNotBeLessThanInitial(
...
pub fn no_retry_refuses_transient_failure_test() {
  ...
  retry.decide(ctx: ctx, failure: retry.Transient)
  |> should.equal(retry.GiveUp(reason: retry.PolicyDisallowsRetry))
}

pub fn no_retry_short_circuits_permanent_failure_test() {
  let ctx = retry.start(policy: retry.no_retry(), seed: 1)

  retry.decide(ctx: ctx, failure: retry.Permanent)
  |> should.equal(
...
pub fn fixed_emits_constant_delay_until_exhausted_test() {
  ...
  let assert retry.Retry(delay: d1, next: ctx1) =
    retry.decide(ctx: ctx0, failure: retry.Transient)
  retry.duration_milliseconds(duration: d1) |> should.equal(250)
  retry.current_attempt(ctx: ctx1) |> should.equal(1)

  let assert retry.Retry(delay: d2, next: ctx2) =
    retry.decide(ctx: ctx1, failure: retry.Transient)
  retry.duration_milliseconds(duration: d2) |> should.equal(250)
  retry.current_attempt(ctx: ctx2) |> should.equal(2)

  retry.decide(ctx: ctx2, failure: retry.Transient)
  |> should.equal(
    retry.GiveUp(reason: retry.MaxAttemptsReached(attempts: 3, limit: 3)),
  )
}
...
pub fn capped_exponential_avoids_overflow_test() {
  ...
  let assert retry.Retry(delay: d, next: _) = drive(ctx0, 50, retry.Transient)
  retry.duration_milliseconds(duration: d) |> should.equal(60_000)
}
...
pub fn unbounded_exponential_overflow_is_reported_test() {
  ...
  let final = drive(ctx0, 54, retry.Transient)
  case final {
    retry.GiveUp(reason: retry.DelayOverflow(at_attempt: at)) ->
      { at >= 54 } |> should.equal(True)
    other ->
      other |> should.equal(retry.GiveUp(reason: retry.PolicyDisallowsRetry))
  }
}
...
pub fn full_jitter_stays_within_zero_to_base_test() {
  ...
  let policy = retry.with_jitter(policy: base_policy, jitter: retry.FullJitter)
...
pub fn equal_jitter_stays_in_lower_half_to_base_test() {
  let assert Ok(base_policy) =
    retry.exponential(initial: ms(1000), multiplier: 2, max_attempts: 6)
  let policy = retry.with_jitter(policy: base_policy, jitter: retry.EqualJitter)
  let ctx = retry.start(policy: policy, seed: 2026)
...
pub fn cumulative_delay_accumulates_test() {
  ...
  retry.duration_milliseconds(duration: retry.cumulative_delay(ctx: ctx3))
  |> should.equal(300)
}
...
pub fn next_delay_returns_delay_or_reason_test() {
  ...
  retry.next_delay(ctx: ctx0, failure: retry.Transient)
  |> should.equal(Ok(ms(50)))

  let assert retry.Retry(delay: _, next: ctx1) =
    retry.decide(ctx: ctx0, failure: retry.Transient)

  retry.next_delay(ctx: ctx1, failure: retry.Transient)
  |> should.equal(Error(retry.MaxAttemptsReached(attempts: 2, limit: 2)))
}
...
pub fn max_attempts_one_gives_up_immediately_test() {
  ...
  retry.decide(ctx: ctx0, failure: retry.Transient)
  |> should.equal(
    retry.GiveUp(reason: retry.MaxAttemptsReached(attempts: 1, limit: 1)),
  )
}
...
pub fn should_retry_matches_decide_test() {
  ...
  retry.should_retry(ctx: ctx0, failure: retry.Transient)
  |> should.equal(True)

  retry.should_retry(ctx: ctx0, failure: retry.Permanent)
  |> should.equal(False)

  let no_ctx = retry.start(policy: retry.no_retry(), seed: 1)
  retry.should_retry(ctx: no_ctx, failure: retry.Transient)
  |> should.equal(False)
}
...
pub fn should_retry_does_not_advance_context_test() {
  ...
  retry.should_retry(ctx: ctx, failure: retry.Transient)
  |> should.equal(True)
  retry.should_retry(ctx: ctx, failure: retry.Transient)
  |> should.equal(True)
...
fn collect_transient_delays(
  ...
  case remaining <= 0 {
    True -> reverse(acc, [])
    False ->
      case retry.decide(ctx: ctx, failure: retry.Transient) {
        retry.Retry(delay: d, next: next) ->
          collect_transient_delays(next, remaining - 1, [
            retry.duration_milliseconds(duration: d),
            ..acc
          ])
